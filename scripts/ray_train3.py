...
from wireless_twin.models.raytrace2 import (build_heightmap, extract_facades,
                                            HeightmapScene, trace_paths3, trace_paths4)
...
P = {}
allp = trace_paths4(scene, bs, torch.tensor(pos, device=dev), max_paths=args.max_paths, max_diff=24)
...
class Assembly(nn.Module):
    def __init__(self):
        ...
        self.g0 = nn.Parameter(torch.zeros(4))            # per-type log-amp
        ...
        self.phi0 = nn.Parameter(torch.zeros(4))          # per-type phase
        ...
        # 垂直虚拟化端口图样: 32仰角bin x MV端口 复数查找表
        self.EB = 32
        self.vpat = nn.Parameter(torch.randn(self.EB, MV, 2) * 0.3)
        ...
        self.wH = nn.Parameter(torch.full((4,), -1.0))
        self.wV = nn.Parameter(torch.full((4,), -1.0))
        self.beta_dang = nn.Parameter(torch.tensor(2.0))
    ...
    def forward(self, p):
        ...
        dang = p.get("dang", torch.zeros_like(tau))
        amp = torch.exp(self.g0[typ] - F.softplus(self.beta_x) * ncx
                        - F.softplus(self.beta_L) * lin
                        - F.softplus(self.beta_dang) * dang) / tau.clamp_min(1.0) ** self.gamma
